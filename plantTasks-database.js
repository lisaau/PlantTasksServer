...
class PlantTasksDatabase {
  ...
  getAllPlants(userId) {
    return this.db.any(
      'SELECT * FROM plants WHERE user_id = $1 ORDER BY id',
      userId
    );
  }

  getPlant(plantId, userId) {
    return this.db.one('SELECT * FROM plants WHERE id = $1 AND user_id = $2', [
      plantId,
      userId,
    ]);
  }
  ...
  addPlant(plantName, plantSpecies, plantNotes, userId) {
    return this.db.one(
      'INSERT INTO plants (name, species, notes, user_id) VALUES($1, $2, $3, $4) RETURNING *',
      [plantName, plantSpecies, plantNotes, userId]
    );
  }
  ...
  editPlant(plantId, plantName, plantSpecies, plantNotes, userId) {
    return this.db.one(
      `UPDATE plants SET name = $2, species = $3, notes = $4 
      WHERE id = $1 AND user_id = $5 RETURNING *`,
      [plantId, plantName, plantSpecies, plantNotes, userId]
    );
  }
  ...
  addTask(description, frequency, plantId, userId) {
    return this.db.one(
      `INSERT INTO tasks (description, frequency, plant_id) 
      SELECT $1, $2, $3
      WHERE EXISTS (SELECT * FROM plants WHERE user_id = $4)
      RETURNING *`,
      [description, frequency, plantId, userId]
    );
  }
  ...
  getTodayTaskInstances(userId) {
    return this.db.any(
      ...
      INNER JOIN tasks AS t ON t.id = ti.task_id
      INNER JOIN plants AS p on p.id = t.plant_id
      WHERE date_trunc('day', ti.due_date) = date_trunc('day', now() AT TIME ZONE 'UTC') 
      AND p.user_id = $1`,
      userId
    );
  }
  ...
  getTaskInstancesByDay(date, userId) {
    return this.db.any(
      ...
      INNER JOIN tasks AS t ON t.id = ti.task_id
      INNER JOIN plants AS p on p.id = t.plant_id 
      WHERE date_trunc('day', ti.due_date) = $1 AND p.user_id = $2)`,
      [date, userId]
    );
  }
    ...
    return this.db
      ...
      .then((task) => {
        ...
        return Promise.all(instancePromises).then((instanceData) => {
          ...
          let taskInstanceIdToday = instanceData.filter(
            (instance) => instance.length !== 0
          )[0][0].id;
          return this.getDetailedTaskInstanceToday(taskInstanceIdToday).then(
            (detailedInstance) => {
              return { task: task, instance: detailedInstance[0] };
            }
          );
        });
      });
  ...
  generateFutureTaskInstances(userId) {
    return this.db
      .any(
        `SELECT DISTINCT all_ti.* 
        FROM (
          SELECT DISTINCT ti.task_id AS null_id 
          FROM task_instances AS ti 
          INNER JOIN tasks AS t ON ti.task_id = t.id
        ...
        WHERE ti_in_future.null_id IS NULL`,
        userId
      )
      .then((tasks) => {
        let instancePromises = tasks.map((task) => {
          return Promise.all(
            [...Array(10).keys()].map((i) => {
              return this.db.one(
                `INSERT INTO task_instances(task_id, due_date)
                VALUES($1, NOW() + $2 * INTERVAL '${i} day') RETURNING *`,
